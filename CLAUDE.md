...
**This is the brpt source repo.** Always use `scripts/dev-brpt.sh` instead of the installed `brpt` command — the installed copy points to the packaged app which may be stale. This applies to all brpt invocations: opening files, sending notifications, etc.
